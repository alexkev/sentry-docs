...
Sentry now allows you to collect user feedback from anywhere inside of your application, without requiring an error event to occur. This requires a minimum SDK version of [7.85.0](#TODO). The previous [Crash Report Widget](#crash-report-widget) still exists to handle user feedback that is associated with an error event.

## User Feedback Widget

Our embeddable JavaScript widget allows your users to submit feedback from anywhere inside of your application. This differs from the [Crash Reports Widget](#crash-report-widget) which is reactive feedback that is specifically tied to an error event.
...
Our embeddable, JavaScript-based, crash report widget is useful when you may typically render a plain error page (the classic `500.html`) on your website.
...
The screenshot below provides an example of the Crash Report widget, though yours may differ depending on your customization:

![An example of a crash report widget with text boxes for user name, email, and additional details about the break.](user_feedback_widget.png)

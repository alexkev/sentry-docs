...
redirect_from:
  - /learn/user-feedback/
  - /platforms/android/enriching-events/user-feedback/
  - /platforms/apple/enriching-events/user-feedback/
  - /platforms/dart/enriching-events/user-feedback/
  - /platforms/dotnet/guides/aspnet/enriching-events/user-feedback/
  - /platforms/dotnet/guides/aspnetcore/enriching-events/user-feedback/
  - /platforms/dotnet/enriching-events/user-feedback/
  - /platforms/elixir/enriching-events/user-feedback/
  - /platforms/flutter/enriching-events/user-feedback/
  - /platforms/go/enriching-events/user-feedback/
  - /platforms/java/enriching-events/user-feedback/
  - /platforms/javascript/enriching-events/user-feedback/
  - /platforms/javascript/guides/react/enriching-events/user-feedback/
  - /platforms/kotlin/enriching-events/user-feedback/
  - /platforms/kotlin-multiplatform/enriching-events/user-feedback/
  - /platforms/kotlin/configuration/options
  - /platforms/native/enriching-events/user-feedback/
  - /platforms/node/enriching-events/user-feedback/
  - /platforms/php/enriching-events/user-feedback/
  - /platforms/python/enriching-events/user-feedback/
  - /platforms/react-native/enriching-events/user-feedback/
  - /platforms/ruby/guides/rails/enriching-events/user-feedback/
  - /platforms/ruby/enriching-events/user-feedback/
  - /platforms/rust/enriching-events/user-feedback/
  - /platforms/unity/enriching-events/user-feedback/
  - /platforms/unreal/enriching-events/user-feedback/
...
Sentry now allows you to collect user feedback from anywhere inside of your application, without requiring an error event to occur. This requires a minimum SDK version of [7.85.0](#TODO). The previous [Crash-Report dialog](#crash-report-dialog) still exists to handle user feedback that is associated with an error event.

## User Feedback Widget

Our embeddable JavaScript widget allows your users to submit feedback from anywhere inside of your application. This differs from the [Crash-Report dialog](#crash-report-dialog) which is reactive feedback that is specifically tied to an error event.
...
By default, this will insert the widget into the bottom right corner of your website. You're free to customize nearly every aspect of the widget, including replacing it completely with your own element.
...
Our embeddable, JavaScript-based, Crash-Report dialog is useful when you may typically render a plain error page (the classic `500.html`) on your website.

To collect feedback, the Crash-Report dialog requests and collects the user's name, email address, and a description of what occurred. When feedback is provided, Sentry pairs the feedback with the original event, giving you additional insights into issues.

The screenshot below provides an example of the Crash-Report dialog, though yours may differ depending on your customization:

![An example of a Crash-Report dialog with text boxes for user name, email, and additional details about the break.](user_feedback_widget.png)

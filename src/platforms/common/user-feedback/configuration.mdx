...
You can skip the default injected button and use your own button to trigger dislaying the form. Call `feedback.attachTo()` to have the SDK attach a click listener to your own button. You can additionally supply the same customization options that the constructor accepts (e.g. for text labels and colors).
...
- a JS object with a required `message` property, and additionally, optional `name` and `email` properties or a `FormData` instance with the same properties
...
You can customize the Crash-Report diawlog to your organization's needs, especially for localization purposes. All options can be passed through the `Sentry.showReportDialog` call.
